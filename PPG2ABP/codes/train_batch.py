...
def train_approximate_network():
    ...
    BATCH_SIZE_PER_REPLICA = 64             # batch size per replica for 4 gpus
    ...
    for foldname in range(10):
        ...
        with strategy.scope():
            ...
            Y_train = prepareLabel(Y_train).batch(GLOBAL_BATCH_SIZE)                                         # prepare labels for training deep supervision

            Y_val = prepareLabel(Y_val).batch(GLOBAL_BATCH_SIZE)                                             # prepare labels for training deep supervision
...
def train_refinement_network():
    ...
    GLOBAL_BATCH_SIZE = BATCH_SIZE_PER_REPLICA * strategy2.num_replicas_in_sync
