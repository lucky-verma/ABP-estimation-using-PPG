...
def train_approximate_network():
    ...
    for foldname in range(10):
        ...
        dt = pickle.load(
            open(os.path.join('data', 'train{}.p'.format(foldname)), 'rb'))
        X_train = dt['X_train']
        Y_train = dt['Y_train']
        # loading validation data
        dt = pickle.load(
            open(os.path.join('data', 'val{}.p'.format(foldname)), 'rb'))
        X_val = dt['X_val']
        Y_val = dt['Y_val']

        # loading metadata
        dt = pickle.load(
            open(os.path.join('data', 'meta{}.p'.format(foldname)), 'rb'))
        max_ppg = dt['max_ppg']
        min_ppg = dt['min_ppg']
        max_abp = dt['max_abp']
        min_abp = dt['min_abp']

        Y_train = prepareLabel(
            Y_train)  # prepare labels for training deep supervision

        Y_val = prepareLabel(
            Y_val)  # prepare labels for training deep supervision

        mdl1 = model_dict[mdlName1](length)  # create approximation network
        
        # Replicates the model on 8 GPUs.
        # This assumes that your machine has 8 available GPUs.
        mdl1 = multi_gpu_model(mdl1, gpus=8)

        # loss = mae, with deep supervision weights
        mdl1.compile(loss='mean_absolute_error',
                     optimizer='adam',
                     metrics=['mean_squared_error'],
                     loss_weights=[1., 0.9, 0.8, 0.7, 0.6])

        checkpoint1_ = ModelCheckpoint(os.path.join(
            'models', '{}_model1_fold{}.h5'.format(mdlName1, foldname)),
                                       verbose=1,
        ...
        history1 = mdl1.fit(X_train, {
            ...
            'level4': Y_train['level4']
                            ...
                            callbacks=[checkpoint1_,WandbCallback()],
                            verbose=1)

        pickle.dump(
            history1,
            open('History/{}_model1_fold{}.p'.format(mdlName1, foldname),
                 'wb'))  # save training history

        mdl1 = None  # garbage collection

        time.sleep(90)  # pause execution for a while to free the gpu
...
def train_refinement_network():
    ...
    for foldname in range(10):
        ...
        mdl1.load_weights(
            os.path.join('models', '{}_model1_fold{}.h5'.format(
                mdlName1, foldname)))  # load weights

        X_train = prepareDataDS(
            mdl1, X_train
        )  # prepare training data for 2nd stage, considering deep supervision
        X_val = prepareDataDS(
            mdl1, X_val
        )  # prepare validation data for 2nd stage, considering deep supervision
        ...
        # Replicates the model on 8 GPUs.
        # This assumes that your machine has 8 available GPUs.
        mdl2 = multi_gpu_model(mdl2, gpus=8)

        # loss = mse
        mdl2.compile(loss='mean_squared_error',
                     optimizer='adam',
                     metrics=['mean_absolute_error'])

        checkpoint2_ = ModelCheckpoint(os.path.join(
            'models', '{}_model2_fold{}.h5'.format(mdlName2, foldname)),
                                       verbose=1,
                                       monitor='val_loss',
                                       save_best_only=True,
                                       mode='auto')
        ...
        history2 = mdl2.fit(X_train,
                            ...
                            callbacks=[checkpoint2_, WandbCallback()])

        pickle.dump(
            history2,
            open('History/{}_model2_fold{}.p'.format(mdlName2, foldname),
                 'wb'))  # save training history
