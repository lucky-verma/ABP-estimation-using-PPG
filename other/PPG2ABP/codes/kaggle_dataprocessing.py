...
def process_data():
    ...
    for k in range(1, 13):  # process for the 4 different parts of the data
        ...
        f = loadmat(os.path.join('raw_data_k', 'part_{}.mat'.format(k)))['p']  # loads the data

        for i in tqdm(range(len(f[0])),desc='Reading Records'):     # reading the records

            signal = f[0][i][0].tolist()     # ppg signal
            bp = f[0][i][1].tolist()              # abp signal

            output_str = '10s,SBP,DBP\n'  # starting text for a new csv file

            for j in tqdm(range(0,len(f[0][i][0])-samples_in_episode, d_samples),desc='Processing Episodes from Record {}/1000'.format(i+1)): # computing the sbp and dbp values
...
def extract_episodes(candidates):
    ...
    for k in tqdm(range(1,13), desc='Reading from Files'):    # iterating throug the files

        # f = h5py.File('./raw_data_k/Part_{}.mat'.format(k), 'r')
        f = loadmat(os.path.join('raw_data_k', 'part_{}.mat'.format(k)))['p']  # loads the data
        ...
        for indix in tqdm(range(len(candidates)), desc='Reading from File {}/12'.format(k)):  # iterating through the candidates
            ...
            for j in tqdm(range(episode_st, episode_st+samples_in_episode), desc='Reading Episode Id {}'.format(indix)):
                ...
                ppg.append(f[0][record_no][0][j]) # ppg signal
                abp.append(f[0][record_no][1][j]) # abp signal
